...
export default function Navbar({ 
  ...
  // Fallback state jika navbar dipanggil tanpa props di Server Component
  ...
  const handleScroll = useCallback(() => {
    ...
    // 1. Mengatur Visibilitas Bar Navigasi (Show/Hide) Berdasarkan Kecepatan Scroll
    if (currentScroll <= 30) {
      setIsAtTop(true);
      setShow(true);
    } else {
  ...
  return (
    <>
          ...
          {show && (
            <motion.nav 
              ...
              className={`pointer-events-auto w-full grid grid-cols-3 items-center rounded-[2.5rem] transition-all duration-500 ${
                isAtTop 
                  ? "max-w-7xl px-10 py-3 bg-transparent text-stone-900" 
            ...
            >
                ...
                <Link href="/" className="flex items-center gap-3">
                  <div className={`transition-all duration-300 group-hover/logo:scale-105 ${
                    ...
                    <Image 
                      ...
                      width={36} 
                      height={36} 
                      className="object-contain filter drop-shadow-[0_2px_8px_rgba(0,0,0,0.1)]" 
                      priority 
                    />
                  </div>
                  <div className="flex flex-col justify-center leading-none">
                    <span className={`font-black text-2xl tracking-tight transition-colors duration-500 ${
                      isAtTop ? "text-stone-900" : "text-[#6F4E37]"
                    }`}>
                    ...
                    <span className={`text-[9px] font-bold tracking-[0.18em] uppercase mt-0.5 transition-colors duration-500 ${
                      isAtTop ? "text-stone-600" : "text-slate-500"
                    }`}>
                      SMKN 3 Manado
                    </span>
                  </div>
              ...
              <div className="flex justify-center">
                <ul className={`flex items-center gap-1 p-1.5 rounded-full transition-all duration-500 ${
                  isAtTop ? "bg-stone-900/5" : "bg-[#6F4E37]/5 border border-[#6F4E37]/10"
                }`}>
                  {menuItems.map((item) => {
                      ...
                      <li key={item.id} className="relative">
                        <Link
                          ...
                          className={`relative z-10 block px-4 py-2 text-xs font-bold tracking-[0.12em] transition-colors duration-300 rounded-full ${
                            isActive 
                              ? "text-white"
                              : isAtTop ? "text-stone-600 hover:text-stone-900" : "text-slate-500 hover:text-[#6F4E37]"
                          }`}
                        >
                        ...
                        {isActive && (
                          <motion.span
                            ...
                            className="absolute inset-0 rounded-full -z-10 shadow-xs"
                            style={{
                              background: isAtTop 
                                ? "linear-gradient(to right, #1c1917, #2e2a24)" 
      ...
      {/* ─── 2. BOTTOM FLOATING DOCK (MOBILE TABS) ─── */}
      <AnimatePresence>
        {show && (
          <motion.div
