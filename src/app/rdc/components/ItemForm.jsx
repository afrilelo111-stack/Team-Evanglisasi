...
export default function ItemForm({
  ...
  // ─── SINKRONISASI PROP KE STATE SECARA AMAN MENGGUNAKAN USEEFFECT ───
  useEffect(() => {
  ...
  return (
    <form
      ...
      className="relative overflow-hidden rounded-[2rem] border border-white bg-white/70 backdrop-blur-2xl shadow-[0_32px_64px_-24px_rgba(59,130,246,0.04)] p-6 md:p-10 space-y-8"
    >
      {/* 🔮 BACKGROUND GLOW EFFECTS */}
      <div className="absolute top-0 right-0 w-80 h-80 bg-blue-500/5 blur-3xl rounded-full pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-80 h-80 bg-sky-500/5 blur-3xl rounded-full pointer-events-none" />
      ...
      <div className="relative flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-100">
        <div className="flex items-center gap-4">
          <div
            className={`relative p-3.5 rounded-2xl shadow-md transition-all duration-300 ${
              isEditing
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-semibold mt-0.5">
              Formulir penayangan portofolio gambar dan sorotan video terintegrasi PPLG 2.
            </p>
          </div>
        </div>

        {isEditing && (
          <div className="self-start sm:self-center inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-amber-50 border border-amber-200 text-amber-700 text-[10px] font-black tracking-widest uppercase animate-pulse">
            <Sparkles size={11} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 items-start">
        ...
        {/* LEFT COLUMN: IMAGE / VIDEO DROPZONE WORKSPACE */}
        <div className="lg:col-span-2 space-y-2.5">
          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">
            File Sampul Foto / Berkas Video
          </label>

          <div className="relative border-2 border-dashed border-slate-200 bg-slate-50/50 hover:border-blue-500 rounded-3xl p-4 flex flex-col items-center justify-center min-h-[300px] overflow-hidden group/dropzone transition-all duration-300 shadow-inner">
            {mediaPreview ? (
              <div className="absolute inset-0 group/preview">
                {mediaType === 'video' ? (
                  ...
                  <Image
                    ...
                    className="object-cover scale-100 group-hover/preview:scale-105 transition-transform duration-500"
                    unoptimized={mediaPreview.startsWith('data:')}
                  />
                )}

                <div className="absolute inset-0 bg-slate-950/60 opacity-0 group-hover/preview:opacity-100 transition-all duration-300 flex flex-col items-center justify-center text-center p-6 backdrop-blur-xs z-10">
              ...
              <div className="text-center p-6 relative z-10 pointer-events-none transition-transform duration-300">
                <div className="w-14 h-14 rounded-2xl bg-white text-slate-400 border border-slate-200 flex items-center justify-center mx-auto shadow-xs mb-3 group-hover/dropzone:text-blue-600 group-hover/dropzone:border-blue-200 transition-colors">
                  <Film size={22} />
                </div>
                <h4 className="text-xs font-black text-slate-800 mb-0.5">Pilih Gambar atau Video</h4>
        ...
        {/* RIGHT COLUMN: INPUT CONTEXT WORKSPACE */}
        <div className="lg:col-span-3 space-y-5">
          
          {/* INPUT KATEGORI: JENIS category */}
          <div className="space-y-2.5">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">
              Klasifikasi Jenis category
            </label>
            <div className="relative group">
              ...
              <select
                ...
                className="w-full bg-white border border-slate-200 rounded-2xl pl-11 pr-5 py-3.5 text-xs font-semibold focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/5 transition-all text-slate-700 appearance-none shadow-xs cursor-pointer"
              >
                {category_OPTIONS.map((opt) => (
                  <option key={opt} value={opt}>{opt}</option>
                ))}
              </select>
              <span className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400 text-[9px]">▼</span>
            </div>
          </div>
          ...
          <div className="space-y-2.5">
            ...
            <div className="relative group">
              <span className="absolute left-4 top-4 text-slate-400 group-focus-within:text-blue-600 transition-colors pointer-events-none">
                <FileText size={16} />
              </span>
              <textarea
                value={description}
          ...
          <div className="flex items-center justify-end gap-2.5 pt-2">
            {isEditing && (
              <button
                ...
                className="inline-flex items-center gap-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-[11px] px-4 py-2.5 rounded-xl transition-all active:scale-95 cursor-pointer"
              >
                <X size={12} />
                <span>Batal</span>
              </button>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className={`relative overflow-hidden inline-flex items-center gap-1.5 font-bold text-[11px] px-4 py-2.5 rounded-xl shadow-md transition-all active:scale-95 text-white disabled:opacity-50 cursor-pointer ${
                isEditing
